...
The ERP (the company's ERP) handles product changes by duplicating the record rather than updating it in place. Every SKU change, option rename, or pricing modification creates a new row in the ERP's `models` table with a fresh ID, and the old row is marked void (prefixed with `^`). The part number -- which Nexus uses as the stable product identifier -- stays the same across versions, but the internal ERP ID changes on every edit.
...
Every new product sync compares the incoming ERP snapshot against the stored version chain. If a new version has appeared (new ID with matching part number), the old version gets `successor_id` set to the new one, `is_current = 0`, and the new version is inserted with `predecessor_id` pointing back.
...
- **Mirror the ERP's own model table structure.** Nexus already stores the raw ERP ID column. The linked list is additive -- it doesn't replace the raw data, it adds relationship metadata that the ERP doesn't expose.
