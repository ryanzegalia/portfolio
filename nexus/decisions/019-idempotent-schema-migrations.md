...
At the time of this decision, Nexus had 145 tables across 58 services. Each service owns its own tables and runs its own inline `CREATE TABLE IF NOT EXISTS` and `ALTER TABLE ADD COLUMN IF NOT EXISTS` at startup. No separate migration tool, no versioned migration files, no "run this command before deploying." The code that uses a table is also the code that creates and evolves it.
