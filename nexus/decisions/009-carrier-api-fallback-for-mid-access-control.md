...
Starting April 1, 2026, USPS implemented MID-based access controls. Queries for a tracking number are authorized only for the MID that issued it. This meant Nexus's direct USPS polling was about to return "access denied" for every the shipping API-issued label -- which is most of them. Hundreds of active shipments were at risk of losing tracking coverage.
...
the shipping API is now a load-bearing integration. A the shipping API outage breaks USPS tracking coverage.
