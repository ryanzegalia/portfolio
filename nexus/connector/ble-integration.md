...
**`bleak`-based asynchronous Bluetooth Low Energy layer** in the Python 3 connector. Used for wireless module configuration and pre-show continuity testing, which happens over BLE (distinct from the SNAP/802.15.4 radio used for the main wireless modules).

## The problem

Wireless modules expose their configuration and test interface over BLE. Operators need to connect to a module, read its continuity and cue status, and record pass/fail results. Multiple modules should be testable in parallel -- waiting for one BLE operation to complete before starting the next is too slow for a real shop floor workflow.
...
- **BLE service UUIDs are module-level constants.** Device info, cue, and script service identifiers are defined as Python UUID constants in `ble_manager.py`. Characteristic UUIDs follow the same pattern. No magic strings scattered through the code.
- **Test session history in SQLite.** `test_records.db` captures every test session with module identity, timestamp, operator, result, and raw cue data. Pushed to the Nexus API via the outbox sync service.
